...
def create_dictionary_mnn(adata, use_rep, batch_name, k = 50, save_on_disk = True, approx = True, verbose = 1, iter_comb = None, batch = True):
    ...
    for comb in iter_comb:
        i = comb[0]
        j = comb[1]
        key_name1 = str(batch_name_df.loc[comb[0]].values[0]) + "_" + str(batch_name_df.loc[comb[1]].values[0])
        ...
        match = mnn(ds1, ds2, names1, names2, knn=k, save_on_disk = save_on_disk, approx = approx, batch=batch)
...
def create_dictionary_mnn_nobatch(adata, use_rep, k = 50, save_on_disk = True, approx = True, verbose = 1):
    cells = adata.obs_names
    mnns = dict()

    mnns['0'] = {} # for multiple-slice setting, the key_names1 can avoid the mnns replaced by previous slice-pair

    ds1 = adata.obsm[use_rep]
    names2 = cells
    # if k>1，one point in ds1 may have multiple MNN points in ds2.
    match = knn(ds1, names2, knn=k)
...
def mnn(ds1, ds2, names1, names2, knn = 20, save_on_disk = True, approx = True, batch = True):
    if approx: 
        ...
        match3 = nn_approx(ds1, ds1, names1, names1, knn=knn)
        match4 = nn_approx(ds2, ds2, names2, names2, knn=knn)
    else:
        match1 = nn(ds1, ds2, names1, names2, knn=knn)
        match2 = nn(ds2, ds1, names2, names1, knn=knn)
        match1 = nn(ds1, ds1, names1, names1, knn=knn)
        match2 = nn(ds2, ds2, names2, names2, knn=knn)
    # Compute mutual nearest neighbors.
    if batch:
        mutual = match1 | set([(b, a) for a, b in match2]) # | match3 | match4
    else:
        mutual = match3 | match4
